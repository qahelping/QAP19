...
def test_find_element(youtube: WebDriver): #  если элемент найден
    assert youtube.find_element(By.ID, 'contents')

def test_find_elements(youtube: WebDriver): #  если элементы найдены
    lst = youtube.find_elements(By.XPATH, '//ytd-guide-entry-renderer')
    assert len(lst) >= 0


def test_find_element_0(youtube: WebDriver): #  если элемент НЕ найден
    try:
...
def test_find_elements_0(youtube: WebDriver): #  если элементы НЕ найдены
    youtube.set_window_size(900, 800)
    lst = youtube.find_elements(By.XPATH, '//ytd-guide-entry-renderer')
    assert lst == []

def test_text(youtube: WebDriver): #  получить текст
    element = youtube.find_elements(By.CSS_SELECTOR, '[id="subtitle"]')[0]
    assert element.text == "Start watching videos to help us build a feed of videos you'll love."
