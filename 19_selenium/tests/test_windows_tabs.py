...
def test_open_windows(browser_practice_automation):
    browser_practice_automation.get(
        "https://practice-automation.com/window-operations/"
    )
    curren_id = browser_practice_automation.current_window_handle

    assert len(browser_practice_automation.window_handles()) == 1
    browser_practice_automation.find_element(
        By.CSS_SELECTOR, "[onclick='newTab()']"
    ).click()
    assert browser_practice_automation.find_element(
        By.CSS_SELECTOR, '[class="wp-block-heading"]'
    ).is_displayed()
    ...
    browser_practice_automation.switch_to.window(
        browser_practice_automation.window_handles()[1]
    )

    assert browser_practice_automation.current_window_handle != curren_id

    assert browser_practice_automation.find_element(
        By.CSS_SELECTOR, '[aria-label="Please allow ads on our site"]'
    ).is_displayed()
